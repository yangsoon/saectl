...
const SAECTL_INSTALLED_PATH = `${HOME_DIR}/.s/components/devsapp.cn/saectl/bin`;
...
export async function installSaeCtl(version: string) {
    ...
    if (!exist) {
        let downloadProgress = spinner("Download plugin");
        try {
    ...
    try {
        await spawn(`tar xf ${downloadFileName} -C ${SAECTL_INSTALLED_PATH}`, { shell: true, stdio: 'inherit' });
    } catch (ex) {
